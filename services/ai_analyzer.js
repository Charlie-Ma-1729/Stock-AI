...
// 🏆 模型調度中心 (雙模型分工)
// ==========================================
// Q8: 邏輯推演強，用於撰寫最終報告與觀點評估
const MODEL_Q8 = 'hf.co/lianghsun/Llama-3.2-Taiwan-3B-Instruct-GGUF:Q8_0'; 
// Q4: 輕量、極速，專職於爬蟲當下的 ETL 新聞濃縮
const MODEL_Q4 = 'hf.co/lianghsun/Llama-3.2-Taiwan-3B-Instruct-GGUF:Q4_0'; 
...
/**
 * [ETL 轉換層] 專職新聞濃縮
 * 說明：由於架構升級，此函數不再負責快取，純粹作為 API 呼叫端。
 * 並指定使用極速的 Q4 模型以節省算力。
 */
async function summarizeNews(title, content) {
    ...
    logger.info(`📰 [新聞濃縮] 準備發送給 Ollama... 標題: ${title.substring(0, 15)}...`);
    
    const startTime = Date.now();
    try {
        const response = await axios.post(OLLAMA_URL, {
            model: MODEL_Q4, // ⚡ 這裡強制使用 Q4 輕量模型
            prompt: prompt, 
            stream: false, 
        ...
        logger.info(`📥 [新聞濃縮] 完成！耗時 ${duration} 秒。 (模型: Q4)`);
...
function buildPrompt(reportType, marketData, compressedNews, userQA, pastMemories, duePredictions) {
    ...
    // 這裡傳入的 compressedNews 已經是 DB 撈出來處理過的心血結晶
    prompt += `【今日核心新聞濃縮】\n`;
    compressedNews.forEach((news, index) => {
        prompt += `${index + 1}. [${news.symbol || '未定'}] ${news.title}\n重點: ${news.compressed_summary || news.summary}\n\n`;
    });
...
async function generateMarketReport(reportType, marketData = {}, compressedNews = []) {
    ...
    logger.info(`📤 [Ollama 連線] 使用模型: ${MODEL_Q8} (高精度)`);
    ...
    try {
        const response = await axios.post(OLLAMA_URL, {
            model: MODEL_Q8, // 🧠 這裡強制使用 Q8 高精度模型
...
async function evaluateUserInput(userName, userInput, type) {
    ...
    if (type === 'viewpoint') {
        ...
        try {
            const response = await axios.post(OLLAMA_URL, {
                model: MODEL_Q8, // 🧠 牽涉到邏輯判定，同樣使用 Q8 模型
                prompt, 
                stream: false, 
